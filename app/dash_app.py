...
def create_volume_speed_scatter(data):
    ...
    fig.update_traces(
        marker=dict(size=10, opacity=0.8, line=dict(width=2, color="DarkSlateGrey"))
    )
    fig.update_layout(
        title_font_size=22,
        legend_title_text="Road",
...
# Callback for traffic volume graph
@app.callback(Output("traffic-volume-graph", "figure"), Input("road-dropdown", "value"))
...
# Callback for volume-speed scatter graph
@app.callback(
    Output("volume-speed-scatter-graph", "figure"), Input("road-dropdown", "value")
)
...
# Callback for peak times bar graph
@app.callback(Output("peak-times-bar-graph", "figure"), Input("road-dropdown", "value"))
